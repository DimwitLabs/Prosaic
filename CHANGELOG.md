...
- Cursor now automatically positions after frontmatter in pieces, after headings in chapters, and at the top in start writing mode. This applies only to newly created files; existing files always open with the cursor at the beginning.

## [1.5.1] - 2026-04-28

### Fixed

- Entering an invalid language code during setup, migration spell-setup, or manage profile no longer silently falls back to en_US. Setup and migration re-prompt until a valid code is entered; the profile modals show an error and block saving until the code is corrected.
